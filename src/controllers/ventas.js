...
const httpVenta = {
    ...
    listarActivas: async (req, res) => {
        try {
            const ventasActivas = await Venta.find({ estado: true });
    ...
    listarInactivas: async (req, res) => {
        try {
            const ventasInactivas = await Venta.find({ estado: false });
    ...
    activar: async (req, res) => {
        try {
            const venta = await Venta.findByIdAndUpdate(req.params.id, { estado: true }, { new: true });
            if (venta) {
                res.json(venta);
            } else {
                res.status(404).json({ message: 'Venta no encontrada' });
    ...
    desactivar: async (req, res) => {
        try {
            const venta = await Venta.findByIdAndUpdate(req.params.id, { estado: false }, { new: true });
            if (venta) {
                res.json(venta);
            } else {
                res.status(404).json({ message: 'Venta no encontrada' });
